...
<template>
  <article class="rounded-lg border border-gray-200 bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
    ...
    <div class="p-4 flex-1 flex flex-col">
      ...
      <p class="mt-2 text-lg font-bold text-gray-900">TK- {{ product.variants.length > 0 ? product.variants[0].selling_price : '-' }}</p>
      <div class="mt-4 flex flex-col sm:flex-row gap-2">
        ...
        >
        <!-- https://a2zbackend.a2zitem.com/api/v1/product/customer/product-details/${product.id} -->
          View Details
        </router-link>
        <!-- <button
          type="button"
